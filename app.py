...
def analyze_multi():
    """Analyze multiple resumes against one job description (batch UI)."""
    try:
        ...
        for resume_file in resume_files:
            ...
            try:
                ...
                batch_results.append({
                    ...
                    'ats_score': match_results['overall_score'],
                    'role_fit_score': ai_analysis['role_fit_score'],
                    'matched_skills_count': len(match_results['matched_flat']),
        ...
        return jsonify({'count': len(batch_results), 'results': batch_results})

    except Exception as e:
        print(f"Batch UI Analysis Error: {e}")
        traceback.print_exc()
        return jsonify({'error': str(e)}), 500
...
Style rules:
- Be concise: 3–7 short bullet points max.
- Avoid long paragraphs. Keep under ~110 words unless the user explicitly asks for a deep explanation.
- Use the analysis context numbers (ATS, role fit, matched/missing skills) to stay specific.
- If the user asks "what should I do", give a clear action checklist.
- If the user asks something unrelated to ResumeIQ Pro or the analysis, politely redirect.
...
- Top matched skills: {', '.join(analysis['matched_skills'][:10])}
- Top missing skills: {', '.join(analysis['missing_skills'][:10])}
- Key strengths: {analysis['ai_analysis']['strengths']}
- Key weaknesses: {analysis['ai_analysis']['weaknesses']}
